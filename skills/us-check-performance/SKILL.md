...
Load `skill://us-concise` before preparing context or findings. This skill is read-only by default and does not authorize application changes, production load, profiler installation, or infrastructure changes.
...
Include a baseline table with the actual environment, workload, results, and limitations. Do not promise a speedup without a controlled before/after comparison. If implementation is later authorized, use `skill://us-workflow` and repeat the same measurement after functional verification. Leave a permanent benchmark only when requested or when it protects a demonstrated regression.
