...
export function runProcess(file, args, options) {
  ...
  if (!options || typeof options !== "object") throw new TypeError("Process options are required.");
  ...
  if (!options.env || typeof options.env !== "object") throw new TypeError("env must be an object.");
  if (options.inheritedFds !== undefined && !Array.isArray(options.inheritedFds)) throw new TypeError("inheritedFds must be an array.");
  ...
  if (options.signal?.aborted) return Promise.reject(processError("Process cancelled.", { cause: options.signal.reason }));
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      if (!failure) failure = processError("Process cancelled.", { cause: options.signal.reason });
      terminate();
    };
    const timeout = setTimeout(() => {
      if (!failure) failure = processError(`Process timed out after ${timeoutMs}ms.`);
      terminate();
    }, timeoutMs);
    ...
    child.once("close", (code, closeSignal) => {
      if (failure) {
        if (!terminating) return finish(failure);
