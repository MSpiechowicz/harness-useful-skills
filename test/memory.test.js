...
test("disabled, queued, zero and malformed saves never report confirmed persistence", async t => {
  ...
  for (const observation of [
    { backend: "off", stored: 0 },
    { backend: "local", stored: 0, queued: true },
    { backend: "local", stored: 0 },
    undefined,
  ]) {
...
test("invalid action, text and extra fields fail before invoking any backend", async t => {
  ...
  const invalid = [
    null, [], {}, { action: "erase" },
    { action: "status", query: "x" }, { action: "build", extra: true },
    { action: "query" }, { action: "query", query: " " }, { action: "query", query: "x".repeat(4097) },
    { action: "search" }, { action: "search", query: 7 },
    { action: "save" }, { action: "save", content: "x".repeat(16385) },
  ];
