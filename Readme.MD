...
**Start here:** [Installation](#installation) · [Claude Code](#claude-code) · [Workflow & skills](#skill-led-development) · [Model configuration](#native-model-configuration) · [Memory](#agent-invoked-memory)
...
Restart OMP after installation or update. Ordinary startup with the installed package adds the discovery hint and, when enabled in the active profile, the narrowly scoped session-local workflow-stage policy. Use `--scope project` for a project installation, or pass `--profile NAME` to both commands for an isolated profile. The repository URL changed, but the OMP marketplace name remains `omp-useful-skills` and the plugin ID remains `oh-my-pi-useful-skills@omp-useful-skills`.
...
Ask naturally: “I want to build a rocket simulator.” With the profile's workflow and automatic stages enabled (the default), `us-workflow` guides the following development flow. Precise requests can require zero clarification questions. Small changes scale the depth without omitting enabled research, reviews, or memory; small size alone does not opt out.
...
Verification failures return to implementation. Repairs repeat verification and whichever automatic reviews are enabled; with both review stages enabled, both reviews repeat. Three unsuccessful repair rounds or repeated no progress stop the loop; enabled memory-stage failures prevent claiming the full workflow complete. Publication is a separate, explicitly authorized action—not the next automatic step.

The parent owns the objective, research scope, native todo list, acceptance, any required approval, and integration. When the research stage is enabled, it uses a read-only worker; when disabled, essential code and caller inspection still happens. When the plan stage is enabled, the native `planner` worker drafts a bounded plan after the parent's inspection and the parent requests approval. Each implementation package is classified as frontend, backend, or genuinely neither. Frontend packages use `frontend`, backend packages use `backend`, and only genuinely neither packages—such as documentation or tooling—use native `task`; every class includes a single package when applicable. Disabling the plan stage removes package-mandatory plan approval, not independently required authorization. Mixed packages split at that boundary where coherent, and dependencies remain serial. Independent ready packages run in parallel only with disjoint ownership. Existing session plans/transcripts support interruption recovery.

With ordinary startup after installation, the extension supplies discovery and the narrowly scoped session-local workflow-stage policy while the **effective workflow** is enabled. Each prompt reconciles these owned instructions without duplicating them; a disabled workflow instead supplies fast-lane guidance and no workflow-stage permission. The enabled-mode discovery instruction permits a native one-request opt-out through `/skill:us-ignore-workflow`, but the extension does not inspect prompt content or skill-shaped markers to detect selection and does not change saved settings for that request. Only when a relevant `us-*` workflow requires an enabled native stage does the policy permit it: a bounded planner draft when the plan stage is enabled, and a selected implementation worker after package approval when required or after any independently required authorization when the plan stage is disabled, including a single package. The parent uses only the active session workspace; the policy never selects a repository globally. It neither authorizes general extra delegation nor starts a workflow, dispatches workers, builds Graphify, or writes workflow state at session open.
...
For a persistent development fast lane, run `/useful-skills workflow disabled` in OMP; `/useful-skills workflow enabled` restores the saved master switch. The choice belongs to the active OMP profile and survives new sessions and restarts. Alternatively, explicitly invoke the native `/skill:us-ignore-workflow` skill for **one request** without changing any saved setting. A copied invocation or skill-shaped marker inside ordinary task material does not change extension guidance. The fast lane still inspects relevant code and callers, implements and exercises the changed path, but skips package-mandatory planning, plan approval, worker delegation, reviews, and automatic memory. It does not bypass stronger safety/permission rules, independently required authorization or requested audits, or publication authorization.

Six profile-wide switches control individual **automatic** stages when workflow is enabled. All default to enabled; changing one leaves the others intact. Use `/useful-skills stage <name> enabled|disabled` with a name from this table:
...
Disabling the workflow master suppresses **all** automatic stages without changing their saved selections. A disabled stage remains disabled when the master is re-enabled; an enabled stage resumes then. One-request `/skill:us-ignore-workflow` takes precedence over the effective workflow and stages for that request only, without persisting a change. Memory opt-outs are guidance, **not** provider/tool bans: explicit `us_memory` actions, `/useful-skills graph test`, `/useful-skills doctor`, and native `/memory` remain available. They do not change native memory configuration or disable host-side `autoRetain`; set that separately if unwanted. Prompt guidance cannot guarantee that a model never invokes an automatic action.

| Skill | Selection and responsibility |
| --- | --- |
| `us-workflow` | Natural build/fix/change/refactor requests; composes the development flow |
| `us-ignore-workflow` | Explicit one-request development fast lane without changing profile settings |
...
| `us-implement` | In-scope implementation and repairs through classified native frontend, backend, or generic workers, subject to applicable authorization |
...
Useful Skills bundles native `planner`, `frontend`, and `backend` agents; it does not add a model router or workflow executor. Ordinary installed startup needs no role mapping, global host override, prompt file, or extra flag for the profile-controlled session-local workflow-stage policy. Existing OMP mappings remain authoritative. If you want to choose distinct mappings, use OMP's native roles in the active profile, project configuration, overlays, or runtime options; Useful Skills never changes them.
...
Loading a skill or defining a role does not execute that role's model. With the workflow effectively enabled, the installed extension's session-local policy permits relevant stages only: with the plan stage enabled, `us-plan` dispatches a bounded `planner` draft after the parent's inspection and the parent requests package approval before implementation; `us-implement` dispatches `frontend`, `backend`, or `task` according to the package classification after that approval. With the plan stage disabled, the package-mandatory planner and approval are skipped, but any independently required authorization remains necessary. Report configured routing separately from actual worker/model metadata when it is available.

After ordinary installation and restart, the package advertises its native agents and defaults to enabled workflow guidance unless the active profile has saved different settings. If an agent is absent, no allowed configured/frontmatter/native route resolves, runtime metadata is unavailable, or stronger host policy prohibits delegation, report the specific limitation rather than silently doing the work in the parent or claiming the requested model ran. A permitted fallback must be identified with its actual routing; it is not proof of the requested role's execution. See [Native model routing](skills/us-workflow/SKILL.md#native-model-routing) for the shared guidance; loading it does not launch the workflow.
...
These management commands run in OMP, not Claude Code:
...
The terminal launcher supports the existing catalog/update operations, plus `install` and `help`; `workflow`, `stage`, `status`, and `graph test` are **in-session OMP commands only**, not terminal subcommands. `/ecc`, `/ecc-doctor`, and `/ecc-memory` aliases are removed. OMP's native `/memory` remains unchanged.

Run `/useful-skills` without arguments for **List**, **Libraries**, **Doctor**, **Workflow**, **Workflow Stages**, **Status**, **Update**, and **Help**. **Workflow** shows the profile's saved master setting and offers **Enabled** and **Disabled**; **Workflow Stages** lists the six automatic-stage switches before choosing a mode. The parenthesized setting status remains inline beside each Workflow or stage label, with only that status shown in a muted color. In the local OMP selector, Left returns from a mode to its parent (from a stage mode to the stage list, then to the root; from Workflow or Update to the root). Escape closes the entire workbench without performing an action; the collaborative remote selector does not forward Left navigation. `/useful-skills status` shows each saved selection alongside its effective value after applying the workflow master; the one-request skill separately overrides guidance for that request without changing the saved selections. Read or write errors are surfaced rather than silently treated as defaults or a successful save. Missing selections default to enabled. Backing out or cancelling a menu performs no operation; invalid workflow/stage arguments leave settings unchanged. Headless command notices do not start a model turn. The graph diagnostic is available only by typing the exact `/useful-skills graph test` command, not from the menu, help, doctor, or startup.
...
The extension reconciles its own discovery and workflow-stage guidance with the active profile's settings on each prompt; native skill expansion supplies explicit one-request skill instructions separately, without changing those settings. The extension cannot authenticate skill selection from prompt text, and its instructions cannot guarantee that a model will ignore a forged skill marker in user-provided material. Depending on OMP's native expansion, an embedded `/skill:us-ignore-workflow` token in copied material may itself select the skill; no trusted selected-skill metadata is available to this hook to distinguish it from deliberate invocation. Switching settings removes stale package guidance while preserving unrelated system instructions. It retains independent catastrophic-command protection and common secret-pattern output redaction, and performs quiet read-only update checks. Opening a session never starts a model workflow, dispatches workers, builds memory, Graphify, or workflow state, or reviews code. `OMP_ECC_SAFETY=off` remains the existing catastrophic-guard opt-out; redaction is not a complete secret detector or a filesystem sandbox.

## Agent-invoked memory

**OMP: two complementary stores, one agent-facing tool.** Facts explain decisions; the code graph maps relationships. Neither replaces the other.
...
With the relevant memory stages effectively enabled, the model follows `us-memory` automatically; the user need not run Graphify or a memory command during an ordinary task. For a deliberate graph diagnostic regardless of those switches, type `/useful-skills graph test` inside OMP. It first announces possible lazy managed setup and profile-local graph writes, builds the current session workspace, then queries the fixed symbol `main` only after a successful build. For example, a repository with `app.py` defining `main` and `helper` can return those nodes and their relationships; other repositories may have no matching `main` symbol. The command reports the actual generation, node/edge/source counts, graph storage path, query text, and returned output. Graphify's `No matching nodes found.` response is explicit, not interpreted as setup failure; an empty response does not claim retrieved relationships. Build/query errors and a changed active generation are reported by stage. Notifications redact common secret patterns and limit display size, but redaction is not complete secret detection. This command does not invoke native memory save/search, start a workflow, or prompt for us_memory tool exec approval: typing it is the explicit request for setup and graph writes. Doctor and other passive commands remain read-only.
...
`update check` is quiet/read-only at interactive startup; explicit failures are reported. `update install` immediately announces that Useful Skills is updating through OMP's native plugin manager, then reports the result. It retains native active-install ownership, scope/profile, and stable-version checks. Only the active unambiguous marketplace installation is upgraded. Once running a release with the corrected release lookup, installed copies receive subsequent published agents only after **Update → Install** and restart.

**Existing OMP installations need a manual recovery after a fixed release is published.** The old updater requests `GET /repos/MSpiechowicz/oh-my-pi-useful-skills/releases/latest`; GitHub returns HTTP 301 after the rename, which the old updater rejects because it uses `redirect: "error"`. That installed code cannot update itself to the fixed version. Do not treat the fix as available before publication. Once the fixed release is published, run these native commands for a user-scoped installation:
...
The first command refreshes the marketplace catalog only; the second upgrades the installed plugin code. Restart OMP afterward. For a project-scoped installation, run both commands from the original project directory and use `--scope project` on the upgrade. For an isolated profile, pass `--profile NAME` to **both** commands (and retain its original scope and working directory). This is not an automatic migration: do **not** run `marketplace add` for the new URL against an existing registration; it fails with a duplicate-name error and does not migrate the saved source URI. The saved source URI may still show the old repository URL; its Git fetch works through GitHub's redirect only while the old repository name remains unused. For a source checkout, changing its Git remote to the new repository URL is optional and user-managed; neither the updater nor these instructions execute that change. The OMP updater does **not** update Claude installations.
...
`graphify.js` orchestrates status/build/query and remains the public entry point for `workspaceMemoryPaths`, `createGraphify`, `graphStatus`, `buildGraph`, and `queryGraph`. `graph-validation.js` owns graph schema, import classification, and source-file policy; `graph-storage.js` owns private storage and active snapshots. Managed setup keeps `dependencies.js` as the public entry point for `createDependencyManager`, `dependencyStatus`, and `ensureDependencies`, with pinned metadata/archive checks in `dependency-policy.js` and bounded downloads in `dependency-transport.js`. Both integrations use `path-boundary.js` for the shared path-containment predicate; their distinct error and symlink policies remain local.

Graph boundary tests are grouped in `test/graph-boundaries.test.js`, `test/graph-validation.test.js`, and `test/graph-transactions.test.js`; extension integration tests are grouped in `test/extension.test.js`, `test/extension-workflow.test.js`, and `test/extension-graph.test.js`. Shared fixtures live in `test/helpers/` and retain per-test cleanup. Keep conditionals as readable blocks and separate distinct decision stages with whitespace. The 500-line cap counts blank lines and comments in authored code and tests, including the `useful-skills` launcher. The pinned, optional ECC archive under `skills/us-library/references/ecc/` is upstream reference material, not an authored-code refactoring target; its oversized files are exceptions to that cap.
