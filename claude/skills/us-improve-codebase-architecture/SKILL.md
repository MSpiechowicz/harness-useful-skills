...
Load `/useful-skills:us-concise` before preparing findings. This skill begins as a read-only architecture investigation. It surfaces candidates and does not authorize refactoring, code changes, documentation changes, publication, or a new workflow run.
...
A report, a top recommendation, and a user's interest in a candidate are not by themselves authorization to edit. Ask which candidate, if any, the user wants to explore and whether they authorize a bounded change. For a selected, authorized candidate, use enabled owned stages directly rather than recursively starting `/useful-skills:us-workflow`:

1. Inspect current source, callers, tests, and conventions. When automatic `research` is enabled, load `/useful-skills:us-research`; enabled backend/graph memory actions during inspection follow `/useful-skills:us-memory` independently.
2. Load `/useful-skills:us-grill-me` only for consequential unresolved decisions; a precise, evidence-backed selection needs no invented questions.
3. When automatic `plan` is enabled, load `/useful-skills:us-plan` to produce one actionable plan and obtain explicit approval before edits. Otherwise a bounded user request can authorize implementation without a mandatory planner or separate plan approval; preserve any independent approval requirement.
4. Once authorized, load `/useful-skills:us-implement`. Preserve established contracts and consumer behavior; migrate affected callers together, maintain meaningful tests, and avoid unrelated cleanup.
5. Obtain fresh correctness review through `/useful-skills:us-review` only if `review` is enabled and security review through `/useful-skills:us-check-security` only if `security_review` is enabled. After verified work and enabled reviews, use `/useful-skills:us-memory` only for enabled backend/graph actions.
