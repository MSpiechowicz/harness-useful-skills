...
Load `/useful-skills:us-concise` before responding. Review only; do not edit, run a workflow, publish, or ask another worker to fix findings.
...
- Do not claim security review; the parent invokes `/useful-skills:us-check-security` separately when its automatic stage is enabled or independently required.
